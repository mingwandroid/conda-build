...
def is_dylib_stub(path):
    result = human_filetype(path)
    return human_filetype(path) == 'DYLIB_STUB'
...
def get_load_commands(lines):
    'yields each load command from the output of otool -l'
    a = 0
    for ln, line in enumerate(lines):
        if line.startswith("Load command") or line.startswith("Terminator"):
            if a < ln:
                # time for a slice :)
                yield lines[a:ln]
            a = ln
...
def get_matching_load_commands(lines, cb_filter):
    '''
    does the grunt work of filtering records and making dicts
    from the output of otool -l
    '''
    result = []
    for lcmds in get_load_commands(lines):
        if cb_filter(lcmds):
            lcdict = {}
            for line in lcmds[1:]:
                listy = line.split()
...
def otool(path, cb_filter=is_dylib_info):
    ...
    Parses the output of the otool -l 'load commands', filtered by
    ...
    The output of otool -l is very freeform, delineation between
    ...
    lines = [line.strip(' \t') for line in
             check_output(['otool', '-l', path]).decode('utf-8').splitlines()]
    lines.append('Terminator')
    return get_matching_load_commands(lines, cb_filter)


def get_dylibs(path):
    "Returns a list of the loaded dylib pathnames"
    dylib_loads = otool(path, is_load_dylib)
    return [dylib_load['name'] for dylib_load in dylib_loads]


def get_id(path):
    "Returns the dylib id name or ''"
...
def get_rpaths(path):
    "Returns a list of the dylib rpaths"
    rpaths = otool(path, is_rpath)
    return [rpath['path'] for rpath in rpaths]


def ldd(path):
    "A barebones non-recursive implementation of Linux's ldd"
    my_dylibs = get_dylibs(path)
    return "\n".join("\t%s => %s (0x0)" % (basename(dylib), dylib)
                for dylib in my_dylibs)


def add_rpath(path, rpath, verbose = False):
    'Adds an `rpath` to the Mach-O file at `path`'
...
def delete_rpath(path, rpath, verbose = False):
    'Deletes an `rpath` from the Mach-O file at `path`'
