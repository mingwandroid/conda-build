...
def is_codefile(filename, skip_symlinks=True):
    if not os.path.isfile(filename) or os.path.getsize(filename) < 4:
        return False
    if skip_symlinks and os.path.islink(filename):
        return False
...
def otool(*args):
    ...
    if args.help:
        print(OTOOL_USAGE)
    if args.machoname:
        shared_libs = inspect_linkages(args.machoname, resolve_filenames=False,
